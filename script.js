...
// ========== API BASE (one place) ==========
const API_BASE = "https://filerdl-website.onrender.com"; // <- your Render URL (https)
...
if (dealerForm) {
  dealerForm.addEventListener('submit', async function (e) {
    ...
    // Required fields check (matches your HTML ids)
    ...
    if (!fullName || !email || !phone || !businessName || !businessType || !address || !city || !state || !pincode || !district || !investment || !space || !monthlyTarget) {
    ...
    // Collect products checkboxes (name="products")
    const checkedBoxes = document.querySelectorAll('input[name="products"]:checked');
    const products = Array.from(checkedBoxes).map(cb => cb.value);
    ...
    try {
      ...
      if (!res.ok) {
        console.error('Dealer server error:', res.status, data);
        alert('Server error: ' + (data?.message || res.status));
        return;
      }

      alert('Dealer application submitted — thank you! We will contact you soon.');
      dealerForm.reset();
    } catch (err) {
